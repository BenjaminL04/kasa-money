...
def make_payment_request(admin_key, bolt11):
    ...
    try:
        ...
        return {"status": "failed", "error": f"Error sending payment request: {e}"}
...
@app.route('/payln', methods=['POST'])
def pay_ln():
    try:
        data = request.get_json()
        if 'token' not in data or 'nonce' not in data or 'signature' not in data or 'bolt11' not in data:
        ...
        payment_result = make_payment_request(admin_key, bolt11)
        return jsonify(payment_result)
    except Exception as e:
        return jsonify({"status": "failed", "error": f"An error occurred: {e}"})
